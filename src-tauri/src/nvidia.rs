use anyhow::{anyhow, Result};
use serde_json::{json, Value};

const NVIDIA_API: &str = "https://integrate.api.nvidia.com/v1/chat/completions";
const NVIDIA_MODELS: &str = "https://integrate.api.nvidia.com/v1/models";

fn system_prompt(mode: &str) -> &'static str {
    match mode {
        "shorten" => "Verilen Türkçe metni anlamını koruyarak kısalt. Sadece düzenlenmiş metni döndür, açıklama yapma.",
        "expand" => "Verilen Türkçe metni aynı dilde, aynı üslupla genişlet ve detaylandır. Sadece metni döndür.",
        "format" => "Verilen metni markdown başlıkları, listeler ve to-do'larla güzel biçimlendir. Sadece markdown döndür.",
        "ocr" => "Verilen metin OCR ile ekrandan okundu, yazim/karakter hatalari iceriyor olabilir. Gorevin:\n\
- Yazim, karakter tanima hatalarini ve bozuk Turkce karakterleri duzelt.\n\
- Satir sonlarini, paragraf yapisini ve girintiyi OLDUGU GIBI koru.\n\
- Anlami, kelime sirasini, icerik ve duzeni KESINLIKLE degistirme; hicbir sey ekleme veya cikarma.\n\
- Cikti SADECE duz metin olsun. HTML, markdown, kod blogu, baslik, liste, aciklama, tirnak, etiket KULLANMA.\n\
- Sadece duzeltilmis duz metni dondur, baska hicbir sey yazma.",
        _ => "Türkçe bir notu hem yazım/dilbilgisi/noktalama olarak düzelt hem de okunabilir biçimde yapılandır. Kurallar:\n\
- Anlamı, üslubu ve içeriği değiştirme; hiçbir şey ekleme veya çıkarma.\n\
- Metin tek bir düşünce/paragrafsa sadece düzeltilmiş paragraf(lar) olarak döndür.\n\
- Birden çok madde, görev, fikir, adım veya yapılacak varsa uygun biçimde yapılandır:\n\
  · Yapılacaklar/aksiyonlar için TipTap task list: <ul data-type=\"taskList\"><li data-type=\"taskItem\" data-checked=\"false\"><p>madde</p></li></ul>\n\
  · Sıralı adımlar için <ol><li>…</li></ol>\n\
  · Bağımsız maddeler için <ul><li>…</li></ul>\n\
  · Gerekirse <h2>/<h3> başlıklarıyla gruplandır.\n\
- Çıktı SADECE geçerli HTML olmalı: <p>, <h2>, <h3>, <ul>, <ol>, <li>, <strong>, <em>, <br>. Kod bloğu, markdown, ``` veya açıklama KULLANMA.\n\
- Cevapta sadece HTML olsun, başka hiçbir metin olmasın.",
    }
}

pub async fn fix_text(text: &str, mode: &str, key: &str, model: &str) -> Result<String> {
    if key.is_empty() {
        return Err(anyhow!("NVIDIA API key tanımlı değil (Ayarlar'dan gir)"));
    }

    let body = json!({
        "model": model,
        "messages": [
            { "role": "system", "content": system_prompt(mode) },
            { "role": "user", "content": text }
        ],
        "temperature": 0.3,
        "top_p": 0.95,
        "max_tokens": 8192,
        "stream": false
    });

    let client = reqwest::Client::new();
    let res = client
        .post(NVIDIA_API)
        .bearer_auth(key)
        .header("Content-Type", "application/json")
        .header("Accept", "application/json")
        .json(&body)
        .send()
        .await?;

    let status = res.status();
    let txt = res.text().await.unwrap_or_default();
    if !status.is_success() {
        return Err(anyhow!("NVIDIA hata {}: {}", status, txt));
    }
    let v: Value = serde_json::from_str(&txt)?;
    let content = v
        .get("choices")
        .and_then(|c| c.get(0))
        .and_then(|c| c.get("message"))
        .and_then(|m| m.get("content"))
        .and_then(|c| c.as_str())
        .ok_or_else(|| anyhow!("NVIDIA cevabı parse edilemedi: {}", txt))?;
    Ok(content.trim().to_string())
}

pub async fn list_models(key: &str) -> Result<Vec<crate::settings::ModelInfo>> {
    if key.is_empty() {
        return Err(anyhow!("NVIDIA API key tanımlı değil"));
    }
    let client = reqwest::Client::new();
    let res = client.get(NVIDIA_MODELS).bearer_auth(key).send().await?;
    let status = res.status();
    let text = res.text().await.unwrap_or_default();
    if !status.is_success() {
        return Err(anyhow!("NVIDIA {}: {}", status, text));
    }
    let v: Value = serde_json::from_str(&text)?;
    let arr = v
        .get("data")
        .and_then(|d| d.as_array())
        .ok_or_else(|| anyhow!("data array bulunamadı"))?;
    let mut out = Vec::with_capacity(arr.len());
    for m in arr {
        let id = m
            .get("id")
            .and_then(|i| i.as_str())
            .unwrap_or("")
            .to_string();
        if !id.is_empty() {
            out.push(crate::settings::ModelInfo {
                name: id.clone(),
                id,
            });
        }
    }
    out.sort_by(|a, b| a.name.to_lowercase().cmp(&b.name.to_lowercase()));
    Ok(out)
}
